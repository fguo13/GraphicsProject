...
struct MyBot {
	...
	std::vector<SkinObject> prepareSkinning(const tinygltf::Model &model) {
		...
		for (size_t i = 0; i < model.skins.size(); i++) {
			...
			computeGlobalNodeTransform(model, localTransforms,model.skins[0].joints[0],
				glm::mat4(1.0f), skinObject.globalJointTransforms);
	...
	void update(float time) {
		...
		if (model.animations.size() > 0) {
			...
			computeGlobalNodeTransform(model, nodeTransforms, rootNodeIndex, glm::mat4(1.0f),
				skinObjects[0].globalJointTransforms);
